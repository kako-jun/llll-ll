...
- **pubkey は hex 固定値**（npub をデコード済み・島では bech32 不要）。relays は `relay.damus.io`/`nos.lol`/`relay.primal.net`（ノート実在を確認した3つ）。
- **各つぶやきはクリックでその投稿へ**（案2）= `https://mypace.llll-ll.com/post/<event.id(hex)>`（`mypace_post_url_base`）。id が 64桁hex のときだけ `<a>` 化。末尾に「続きを見る」= `mypace_user_url`（ユーザーページ）。
- **本文は textContent**（innerHTML 禁止＝XSS/世界観防止）、CSS line-clamp で数行 truncate。
